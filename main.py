...
def read_html_file(filepath: str) -> str:
    """Read HTML or MHTML files, returning the HTML content as a string."""
    with open(filepath, "rb") as f:
        raw = f.read()

    # Detect MHTML by checking for MIME headers
    if raw[:5] == b"From " or b"MIME-Version:" in raw[:500]:
        ...
        # Fallback: decode the whole thing
    ...
    for html_file in html_files:
        ...
        try:
            ...
            if source == "udemy":
                ...
                questions = scrape_examtopics_page(
                    html_content, Path(html_file).name
                )
...
def create_anki_csv(
    ...
    with open(output_file, "w", encoding="utf-8", newline="") as f:
        ...
        if truncated_count > 0:
            print(
                f"  - Questions with >{MAX_ANSWERS} choices (reordered):"
                f" {truncated_count}"
            )
        print(f"{'='*60}")
...
def find_html_files() -> List[str]:
    files = glob.glob("*.html") + glob.glob("*.mhtml")
